...
class SQLPrompt(BasicPrompt):
    ...
    def format_question(self, example: dict):
        ...
        if "evidence" in example.keys():
            prompt_note = self.template_note.format(example["evidence"])
            if prompt_extra_info is None or prompt_extra_info == "":
                prompt_components = [prompt_info,  prompt_question, prompt_note]
            else:
                prompt_components = [prompt_info, prompt_extra_info, prompt_question,  prompt_note]
        else:
            if prompt_extra_info is None or prompt_extra_info == "":
...
class OpenSQLPrompt(BasicPrompt):
    template_info = """### Complete sqlite SQL query only and with no explanation
### SQLite SQL tables are requested to be represented in the following format.
# 
# TABLE_NAME (
#   COLUMN_NAME:  TYPE, DESCRIPTION, VALUES, PRIMARY_KEY        
#   ...
# )
# ...
# FOREIGN KEYS:
...
"""
    def format_question(self, example: dict):
        schema = example["schema_info"]
        # foreign_keys
        prompt = self.template_info.format(table_info =schema, question = example["question"], note = example['evidence'] )
        # print(prompt)
        return prompt
    
    
class OpenSQLPrompt_SPIDER(BasicPrompt):
    template_info = """### Complete sqlite SQL query only and with no explanation
### SQLite SQL tables are requested to be represented in the following format.
# 
# TABLE_NAME (
#   COLUMN_NAME:  TYPE, VALUES, PRIMARY_KEY        
#   ...
# )
# ...
# FOREIGN KEYS:
...
"""
    def format_question(self, example: dict):
        schema = example["schema_info"]
        # foreign_keys
        prompt = self.template_info.format(table_info =schema, question = example["question"] )
        # print(prompt)
        return prompt