...
from utils.utils import sql2skeleton, jaccard_similarity, calculate_jaccard_similarity
...
class EuclideanDistancePreSkeletonQuestionMaskSimilarThresholdSelector(BasicExampleSelector):
    def __init__(self, data, *args, **kwargs):
        super().__init__(data)
        ...
        self.bert_model = SentenceTransformer(self.SELECT_MODEL, device="cpu")
        self.train_embeddings = self.bert_model.encode(train_mask_questions)
        print("BERT INIT SUCESS!")

    def get_examples(self, target, num_example, cross_domain=False):
        target_mask_question = mask_question_with_schema_linking([target], mask_tag=self.mask_token, value_tag=self.value_token)
        target_embedding = self.bert_model.encode(target_mask_question)
        #find the most similar pre skeleton in train dataset
        train_json = self.train_json
        train_skeleton_list = [ _["pre_skeleton"] for _ in train_json ]
        distances = calculate_jaccard_similarity(target["pre_skeleton"], train_skeleton_list)
        pairs = [(distance, index) for distance, index in zip(distances, range(len(distances)))]
        ...
        train_json = self.train_json
        pairs_sorted = sorted(pairs, key=lambda x: x[0])[::-1]
        top_pairs = list()
        for d, index in pairs_sorted:
            similar_db_id = train_json[index]["db_id"]
            if cross_domain and similar_db_id == target["db_id"]:
                continue
            # EuclideanDis 
            distance = euclidean_distances(target_embedding, [self.train_embeddings[index]])[0]
            # print(distance)
        ...
        if len(top_pairs) < num_example:
            for d, index in pairs_sorted:
                ...
                if cross_domain and similar_db_id == target["db_id"]:
                    continue
                # EuclideanDis 
                # print(self.train_embeddings[index].shape)
                distance = euclidean_distances(target_embedding, [self.train_embeddings[index]])[0]
                # print(distance)
...
class EuclideanDistancePreSkeletonQuestionMaskSimilarThresholdShiftSelector(BasicExampleSelector):
    def __init__(self, data, *args, **kwargs):
        super().__init__(data)
