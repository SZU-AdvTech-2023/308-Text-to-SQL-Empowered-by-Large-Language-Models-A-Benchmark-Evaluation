...
class BasicDataset(object):
    def __init__(self, path_data, pre_test_result=None):
        ...
        if self.schema_json :
            self.schema_json = os.path.join(self.path_data, self.schema_json)
        ...
        self.databases = None
        self.foreign_keys = self.get_foreign_keys_dict()
    ...
    def get_foreign_keys_dict(self):
        total_data = json.load(open(self.table_json))
        foreign_keys = {}
        for data in total_data:
            ...
            foreign_keys_list = data["foreign_keys"]
            for item in foreign_keys_list:
                try:
                    key1, key2 = item[0], item[1]
                    item1 , item2 = columns[key1], columns[key2]
                    table1, table2 = tables[item1[0]], tables[item2[0]]
                    column1 , column2 = item1[1], item2[1]
                    pattern1 = "{}.{}".format(table1, column1)
                    pattern2 = "{}.{}".format(table1, column1)
                    foreign_keys[db_name][pattern1 ] = pattern2
                except:
                    continue
        return foreign_keys
    ...
    def get_schema_info(self, db_id):
        table_data = json.load(open(self.schema_json))[db_id]
        schema_info = "#\n"
        table_format = "# {table_name}(\n{columns_info}# )\n"
        foreign_keys =  self.foreign_keys[db_id]
        ...
        for key, value in table_data.items():
            ...
            for col, col_value in value.items():
                columns_info += "#   " + col_value + "\n"
            schema_info += table_format.format(table_name=key, columns_info=columns_info)
        #foreign keys info 
        foreign_keys_set = set()
        foreign_keys_info = "# FOREIGN KEYS:\n"
        for key, value in foreign_keys.items():
            if value not in foreign_keys_set or key not in foreign_keys_set:
                foreign_keys_set.add(key)
                foreign_keys_info += "#   " +  key + " = " + value + "\n"
                foreign_keys_set.add(value)
        schema_info = schema_info + foreign_keys_info + "#" 
        # print(schema_info)   
        return schema_info
    ...
    def get_train_questions(self):
        if self.max_train_questions:
            questions = json.load(open(self.train_json, "r"))[:self.max_train_questions]
        else: 
    ...
    def get_train_json(self):
        if self.max_train_questions:
            datas = json.load(open(self.train_json, "r"))[:self.max_train_questions]
        else:
    ...
    def get_test_json(self, mini_set=False):
        ...
        if self.pre_test_result:
            ...
            pre_queries = None
        return self.data_pre_process(tests, linking_infos, pre_queries,"test")
    ...
    def data_pre_process(self, datas, linking_infos=None, pre_queries=None, data_type='train'):
        db_id_to_table_json = dict()
        for table_json in self.get_table_json():
            db_id_to_table_json[table_json["db_id"]] = table_json
        for idx, data in enumerate(datas):
            ...
            try:
                ...
                if data['question'] == "List down 5 non English adventure movies from UK?":
                    print('query')
                    print(data["query_skeleton"])
            except Exception as e:
                data['query_skeleton'] = ""
            data["path_db"] = self.get_path_db(db_id)
               
            if self.schema_json and data_type == 'test':
                data['schema_info'] = self.get_schema_info(db_id)
        if linking_infos:
            db_id_to_table_json = dict()
            for table_json in self.get_table_json():
                db_id_to_table_json[table_json["db_id"]] = table_json
...
class BIRDDataset(BasicDataset):
    name = "bird"
    test_json = "dev/dev.json"
    test_gold = "dev/dev_gold.sql"
    train_json = "train/train_check.json"
    train_gold = "train_check_gold.sql"
    table_json = "tables.json"
    schema_json = "schema_all.json"
    mini_test_index_json = None
    max_train_questions = 5807
