...
class PreprocessedSchema:
    ...
    # only for BIRD dataset
    # column_ = attr.ib(factory=list)
...
class BIRDEncoderV2Preproc(abstract_preproc.AbstractPreproc):
    ...
    def add_item(self, item, schema, section, validation_info):
        preprocessed = self.preprocess_item(item, schema, validation_info)
        self.texts[section].append(preprocessed)
        with open(os.path.join(self.data_dir, section + '_schema-linking.jsonl'), 'a') as f:
    ...
    def preprocess_item(self, item, schema, validation_info):
        ...
        question, question_for_copying = self._tokenize_for_copying(item['question'])
        print("Now is ", datetime.datetime.now())
        print("Tokenize finish!")
        preproc_schema = self._preprocess_schema(schema)
        print("Now is ", datetime.datetime.now())
        print("Schema process finish!")
    ...
    def _tokenize_for_copying(self, unsplit):
        if self.word_emb:
            return self.word_emb.tokenize_for_copying(unsplit)
        return upsplit.split(","), upsplit.split(",")
